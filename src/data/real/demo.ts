...
export const demoFilings = (symbol: StockSymbol): DataEnvelope<FilingData[]> => envelope(filings[symbol].map((filing) => ({
  ...
  reportDate: null, companyId: ids[symbol], company: companyBySymbol[symbol].name, cik: ciks[symbol], primaryDocument: "Illustrative", canonicalUrl: null, source: "Demo data",
})));
