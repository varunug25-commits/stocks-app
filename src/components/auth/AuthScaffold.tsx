...
export function AuthScaffold({ title, description, children, footer, back = true }: PropsWithChildren<{ title: string; description: string; footer?: ReactNode; back?: boolean }>) {
  return <AppScreen keyboard padded scroll><AppHeader back={back} /><Animated.View entering={FadeInDown.duration(380)} style={styles.body}>{publicDataConfig.mode === "DEMO" ? <DemoDataBadge /> : null}<Text style={styles.title}>{title}</Text><Text style={styles.description}>{description}</Text><View style={styles.form}>{children}</View>{footer}</Animated.View></AppScreen>;
}
